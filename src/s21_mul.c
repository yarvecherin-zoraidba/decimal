...
static void s21_big_zero(unsigned big[], int n) {
  for (int i = 0; i < n; ++i) big[i] = 0u;
}

static unsigned s21_big_inc(unsigned big[], int n) {
...
static unsigned s21_big_div10(unsigned big[], int n) {
...
static unsigned s21_big_mod10_const(const unsigned big[], int n) {
  unsigned long long rem = 0;
  for (int i = n - 1; i >= 0; --i) {
    unsigned long long cur = (rem << 32) | (unsigned long long)big[i];
    rem = cur % 10u;
...
static int s21_big_fits_96(const unsigned big[]) {
...
static void s21_big_round_div10_bankers(unsigned big[], int n, unsigned rem,
                                        int sticky) {
  if (rem > 5u) {
...
int s21_mul(s21_decimal a, s21_decimal b, s21_decimal *result) {
...

  {
    ...
    while (!s21_big_fits_96(big)) {
      if (scale == 0u) {
        return sign ? S21_INF_NEG : S21_INF_POS;
      }
      unsigned rem = s21_big_div10(big, S21_LIMB_COUNT_192);
      s21_big_round_div10_bankers(big, S21_LIMB_COUNT_192, rem, sticky);
      if (rem != 0u) sticky = 1;
